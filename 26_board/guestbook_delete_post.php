...
// $check_sql = "SELECT * FROM guestbook_posts WHERE id='$post_id'"; // 해당 ID의 게시물을 조회하는 SQL 쿼리
...
// $file_sql = "SELECT * FROM guestbook_attachments WHERE post_id='$post_id'"; // 해당 게시물에 첨부된 파일을 조회하는 SQL 쿼리
...
//guestbook_attachments 테이블에서 해당 게시물과 관련된 파일 정보를 삭제하는 SQL 쿼리
// $delete_file_sql = "DELETE FROM guestbook_attachments WHERE post_id='$post_id'";
...
// $delete_comment_sql = "DELETE FROM guestbook_comments WHERE post_id='$post_id'";
// mysqli_query($connection, $delete_comment_sql); // 쿼리 실행
$stmt = mysqli_prepare(
    $connection,
    "DELETE FROM guestbook_comments WHERE post_id=?"
);
...
// $delete_post_sql = "DELETE FROM guestbook_posts WHERE id='$post_id'";
$stmt = mysqli_prepare(
    $connection,
    "DELETE FROM guestbook_posts WHERE id=?"
);
...
if (mysqli_stmt_execute($stmt)) {
    ...
    echo "<script>
        alert('게시물이 삭제되었습니다.');
        location.href='guestbook_index.php';
    </script>";
    exit();
} else {
    die("게시물 삭제에 실패했습니다.");
