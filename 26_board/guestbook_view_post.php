...
$sql = "SELECT * FROM guestbook_posts WHERE id='$post_id'"; // 해당 ID의 게시물을 조회하는 SQL 쿼리
...
<html>
    ...
    <body>
        <a href="guestbook_index.php">목록으로 돌아가기</a> <!-- 게시물 상세보기 페이지에서 게시물 목록 페이지로 돌아가는 링크를 제공한다. -->
        <h1><?php echo $post['title']; ?></h1> <!-- 게시물의 제목을 출력한다. -->
        <small> <!-- 게시물의 작성자와 작성일을 작은 글씨로 출력한다. -->
            작성자:
            <a href="guestbook_index.php?writer=<?php echo $user['username']; ?>">
        ...
        $file_sql = "SELECT * FROM guestbook_attachments WHERE post_id='$post_id'";
        ...
        if ($post['author_id'] == $_SESSION['user_id']) {
            ...
            <a href="guestbook_edit_post.php?id=<?php echo $post['id']; ?>">수정</a>
            <!-- 게시물 삭제 페이지로 이동하는 링크를 제공한다. URL에 게시물 ID를 전달하여 해당 게시물을 삭제할 수 있도록 한다. -->
            <a href="guestbook_delete_post.php?id=<?php echo $post['id']; ?>">삭제</a>
            <?php
        }
        ?>
        ...
        <h2>댓글 작성</h2>
        <!-- <form>은 입력창 -->
        <form action="guestbook_comment_process.php" method="post">
        ...
        $edit_com_id = isset($_GET['edit_com_id']) ? $_GET['edit_com_id'] : null;

        $sql = "SELECT * FROM guestbook_comments WHERE post_id='$post_id' ORDER BY created_at DESC"; // 해당 게시물의 댓글을 조회하는 SQL 쿼리
        $result = mysqli_query($connection, $sql);
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            while($comment = mysqli_fetch_assoc($result)) {
                ...
                if ($comment['id'] == $edit_com_id) {
                    ...
                    ?>
                    <form action="guestbook_edit_comment_process.php" method="post">
                        ...
                        <textarea name="content"><?php echo $comment['content']; ?></textarea><br>
                        <input type="submit" value="완료">
                        <a href="guestbook_view_post.php?id=<?php echo $post_id; ?>">취소</a>
                    </form>
                    <?php
                } else {
                    // 2. 아니면 평순한 텍스트로 보여준다.
                    ...
                    ?>
                    <a href="guestbook_view_post.php?id=<?php echo $post_id; ?>&edit_com_id=<?php echo $comment['id']; ?>">수정</a>

                    <a href="guestbook_delete_comment.php?id=<?php echo $comment['id']; ?>">삭제</a>
                    <br><br>
                    <?php
                    }
                }
